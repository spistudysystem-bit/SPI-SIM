...
const CATEGORIES_MAP = [
  {
    ...
    badge: "Sound Engine",
    items: [
      { id: 'probe' as const, label: 'Internal Probe', desc: 'Piezoelectric damping, crystal parameters, matching layers', icon: Layers },
      { id: 'types' as const, label: 'Clinical Arrays', desc: 'Linear, curvilinear, and phased-array transducer geometries', icon: Compass },
      { id: 'beam' as const, label: 'Beam Formation', desc: 'Huygens principle, wave interference, and electrical focusing', icon: Zap },
      { id: 'pulse' as const, label: 'Pulsed Wave', desc: 'Duty factor, PRP, PRF, spatial pulse length equations', icon: Activity },
    ]
  },
  {
    ...
    badge: "Boundaries",
    items: [
      { id: 'interactions' as const, label: 'Refraction & Reflection', desc: 'Impedance mismatch boundaries and reflection math', icon: Zap },
      { id: 'attenuation' as const, label: 'Decibel Loss Sim', desc: 'Depth-dependent tissue attenuation physics coefficients', icon: Activity },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'doppler' as const, label: 'Spectral Doppler', desc: 'Acoustic frequency shifting and velocity pulse graphics', icon: Play },
    ]
  },
  {
    ...
    items: [
      ...
      { id: 'safety' as const, label: 'Scanner Safety', desc: 'Bioeffects, Mechanical, and Thermal Safety indices (ALARA)', icon: ShieldAlert },
    ]
  },
  {
    ...
    items: [
      { id: 'academy' as const, label: 'Interactive Textbook', desc: 'Complete board-ready ARDMS/SPI physics reference text', icon: Book },
      { id: 'quest_station' as const, label: 'Gamified Quests', desc: 'Comprehensive medical challenges and scoring puzzles', icon: Sparkles },
      { id: 'practice' as const, label: 'Syllabus Exam', desc: 'Real-time full multiple choice simulator & grading system', icon: GraduationCap },
      { id: 'chat' as const, label: 'Ask AI Board Assistant', desc: 'Conversational cognitive clinical instructor', icon: Sparkles },
      { id: 'library' as const, label: 'Voice Lectures', desc: 'Audio lecture tracks with dynamic clinical media clips', icon: Award },
      { id: 'profile' as const, label: 'Operator Profile', desc: 'Study levels, scoring records, and credential checklist', icon: UserIcon },
...
type ViewMode = 'probe' | 'types' | 'pulse' | 'beam' | 'physics' | 'doppler' | 'interactions' | 'attenuation' | 'hemodynamics' | 'imaging' | 'artifacts' | 'safety' | 'practice' | 'library' | 'chat' | 'academy' | 'profile' | 'roadmap' | 'quest_station';
...
export default function App() {
  ...
  const [menuHubOpen, setMenuHubOpen] = useState(false);
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full bg-[#0c0d10] text-[#e0e0e0] font-sans selection:bg-[#00d1ff]/30 overflow-hidden hud-dots">
      <HUDOverlay />
      <header className="h-14 lg:h-16 border-b border-[#2d3139] flex items-center justify-between px-4 sm:px-8 bg-[#16181d]/90 backdrop-blur-xl z-50 shrink-0">
        <div className="flex items-center justify-between w-full lg:w-auto gap-4 lg:gap-6">
          <div className="flex items-center gap-3 sm:gap-6 shrink-0">
            <button 
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className={`p-2 rounded-full transition-all duration-300 ${sidebarOpen ? 'bg-[#00d1ff]/10 text-[#00d1ff]' : 'text-[#8e9299] hover:text-white hover:bg-white/5'}`}
            >
              {sidebarOpen ? <X size={18} /> : <Menu size={18} />}
            </button>
            <div className="flex items-center gap-2 sm:gap-3 cursor-pointer group" onClick={() => setViewMode('probe')}>
              <div className="w-6 h-6 sm:w-8 sm:h-8 bg-[#00d1ff] rounded-sm flex items-center justify-center shadow-[0_0_20px_rgba(0,209,255,0.3)] group-hover:shadow-[0_0_30px_rgba(0,209,255,0.5)] transition-all">
                <Zap size={14} className="text-black sm:w-[18px] sm:h-[18px]" />
              </div>
              <div className="brand text-[12px] sm:text-[16px] tracking-[2px] sm:tracking-[4px] uppercase text-white font-light">
                Sonic<span className="serif-italic font-bold text-[#00d1ff]">Build</span>
              </div>
            </div>
          </div>
        ...
        <div className="hidden lg:flex relative w-full lg:max-w-[55vw] xl:max-w-[50vw] items-center overflow-hidden">
          {/* Subtle fade overlay cues on mobile */}
          <div className="absolute left-0 top-0 bottom-0 w-4 bg-gradient-to-r from-[#16181d] to-transparent pointer-events-none z-10 block lg:hidden" />
          
          <div className="flex bg-[#0c0d10] p-1 rounded-full border border-[#2d3139] gap-0.5 overflow-x-auto no-scrollbar w-full shadow-inner shadow-black/50">
            {tabs.map(tab => (
              <button 
                key={tab.id}
                onClick={() => setViewMode(tab.id)}
                className={`px-4 py-2 text-[8px] uppercase font-bold tracking-widest rounded-full transition-all shrink-0 relative ${viewMode === tab.id ? 'bg-[#00d1ff] text-black shadow-lg shadow-[#00d1ff]/20' : 'text-[#8e9299] hover:text-white hover:bg-white/5'}`}
              >
                {tab.label}
                {viewMode === tab.id && <motion.div layoutId="active-tab" className="absolute inset-0 bg-[#00d1ff] rounded-full -z-10" />}
              </button>
            ))}
          </div>

          <div className="absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-[#16181d] to-transparent pointer-events-none z-10 block lg:hidden" />
        </div>
      ...
      <div className="lg:hidden flex items-center justify-between px-4 py-2 bg-[#0b0c0f] border-b border-[#2d3139]/80 text-[#8e9299] font-mono text-[9px] select-none shrink-0 overflow-x-auto no-scrollbar gap-4 shadow-inner">
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[#00d1ff] font-extrabold tracking-wider">f₀:</span>
          <span className="text-white font-bold">{frequency.toFixed(2)}<span className="text-[7.5px] text-slate-500 font-normal">MHz</span></span>
        </div>
        <div className="w-[1px] h-3.5 bg-white/10 shrink-0" />
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-amber-400 font-extrabold tracking-wider">λ:</span>
          <span className="text-white font-bold">{wavelength.toFixed(3)}<span className="text-[7.5px] text-slate-500 font-normal">mm</span></span>
        </div>
        <div className="w-[1px] h-3.5 bg-white/10 shrink-0" />
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-rose-400 font-extrabold tracking-wider">AR:</span>
          <span className="text-white font-bold text-emerald-400">{axialRes.toFixed(2)}<span className="text-[7.5px] text-slate-500 font-normal">mm</span></span>
        </div>
        <div className="w-[1px] h-3.5 bg-white/10 shrink-0" />
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-emerald-400 font-extrabold tracking-wider">R:</span>
          <span className={`font-bold ${reflectionCoeff > 0.8 ? "text-red-400 animate-pulse" : "text-white"}`}>{((reflectionCoeff) * 100).toFixed(1)}%</span>
        </div>
        <div className="w-[1px] h-3.5 bg-white/10 shrink-0" />
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-purple-400 font-extrabold tracking-wider">SYS_MED:</span>
          <span className="text-white font-extrabold uppercase tracking-tight">{activeMedium.name} <span className="text-[7px] text-slate-500 font-normal font-mono">({activeMedium.c * 1000}m/s)</span></span>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          {sidebarOpen && (
            <>
              ...
              <motion.aside 
                ...
                className="absolute lg:relative inset-y-0 left-0 border-r border-[#2d3139] bg-[#0f1115] flex flex-col gap-6 overflow-hidden shrink-0 z-40 w-[85vw] max-w-[300px] sm:w-[360px] sm:max-w-none"
              >
                <div className="p-5 sm:p-8 flex flex-col gap-6 h-full overflow-y-auto w-full">
                  <div className="flex justify-between items-center lg:hidden -mb-2">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="p-2 text-[#8e9299]">
                      <X size={20} />
                    </button>
                  </div>
        ...
        <main className={`flex-1 relative bg-[#0c0d10] flex flex-col overflow-y-auto overflow-x-hidden xl:overflow-y-auto scroll-smooth ${isWorkspaceFullScreen ? 'fixed inset-0 z-[120] bg-black p-4 md:p-8' : ''}`}>
          ...
          <div className="lg:hidden flex flex-col border-t border-[#2d3139] bg-[#16181d] sticky bottom-0 z-30 shadow-[0_-12px_24px_rgba(0,0,0,0.65)] shrink-0">
            ...
            <div 
              ref={bottomBarRef}
              className="flex items-center gap-2 overflow-x-auto no-scrollbar px-4 py-2.5 bg-[#0b0c10] border-b border-[#2d3139]/40 w-full"
            >
              <button 
                onClick={() => {
                  setMenuHubOpen(true);
                ...
                className="px-3 py-1.5 bg-gradient-to-r from-[#00d1ff]/20 to-indigo-500/10 hover:from-[#00d1ff]/30 border border-[#00d1ff]/50 text-[#00d1ff] rounded-lg transition-all shrink-0 font-mono text-[9px] font-black tracking-widest flex items-center gap-1 shadow-[0_0_12px_rgba(0,209,255,0.15)] select-none cursor-pointer"
              >
                <LayoutGrid size={11} className="text-[#00d1ff] shrink-0 animate-pulse" />
                <span>INDEX HUB</span>
              </button>
              
              <div className="w-[1px] h-4 bg-[#232731] shrink-0 mx-1" />
              ...
              {tabs.map(tab => (
                <button 
                  ...
                  className={`px-3.5 py-1.5 text-[8.5px] uppercase font-bold tracking-widest rounded-lg transition-all shrink-0 whitespace-nowrap border ${viewMode === tab.id ? 'bg-[#00d1ff] text-black border-[#00d1ff] shadow-[0_0_10px_rgba(0,209,255,0.35)]' : 'bg-transparent border-[#2d3139]/50 text-[#8e9299] hover:text-white'}`}
                >
                  {tab.label}
                </button>
              ))}
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-[#12141a]">
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => setSidebarOpen(!sidebarOpen)}
                  className={`flex items-center gap-1 text-[9px] uppercase font-mono tracking-wider font-bold rounded-lg px-2.5 py-1.5 transition-all border shrink-0 cursor-pointer ${sidebarOpen ? 'bg-[#00d1ff]/15 border-[#00d1ff]/40 text-[#00d1ff]' : 'bg-white/5 border-white/10 text-[#8e9299]'}`}
                >
      ...
      {/* Mobile Index Command Center Dialog */}
      <AnimatePresence>
        {menuHubOpen && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <motion.div 
              ...
              className="mt-auto w-full max-h-[92dvh] bg-[#0c0d10] border-t border-[#2d3139] rounded-t-3xl shadow-[0_-15px_40px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="bg-[#14161d] p-5 border-b border-[#2d3139]/60 flex flex-col gap-4 shrink-0">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2.5">
                    <div className="p-1.5 rounded-lg bg-[#00d1ff]/10 border border-[#00d1ff]/30 text-[#00d1ff]">
                      <LayoutGrid size={16} />
                    </div>
                    <div>
                      <h4 className="text-sm font-black font-mono text-white tracking-widest uppercase text-left">SonicBuild Hub</h4>
                      <p className="text-[10px] text-[#8e9299] font-mono text-left">19 INTERACTIVE ULTRASOUND SIMULATORS</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setMenuHubOpen(false)}
                    className="p-2 rounded-full bg-white/5 border border-[#2d3139]/60 text-white hover:bg-white/10"
                  >
                    <X size={16} />
                  </button>
                </div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none text-[#8e9299]">
                  ...
                  <input 
                    ...
                    placeholder="Search pipelines (e.g. TGC, Doppler, Exam)..."
                    className="w-full pl-10 pr-10 py-2.5 bg-[#0b0c10] border border-[#2d3139] text-[#e0e0e0] rounded-xl text-xs placeholder-[#525969] focus:outline-none focus:border-[#00d1ff] transition-all"
                  />
                  {searchText && (
                    <button 
                      onClick={() => setSearchText('')}
                      className="absolute inset-y-0 right-3 flex items-center text-[#8e9299] hover:text-white"
                    >
                      <X size={14} />
                    </button>
                  )}
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-10">
                  ...
                  return (
                    <div key={catIdx} className="space-y-3">
                      <div className="flex items-center justify-between border-b border-[#2d3139]/40 pb-1">
                        ...
                        <span className="text-[8px] font-mono px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-slate-400">
                          {category.badge}
                        </span>
                      </div>
                        ...
                        {filteredItems.map(item => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                setViewMode(item.id);
                                setMenuHubOpen(false);
                              }}
                              className={`w-full p-3.5 text-left rounded-xl border transition-all flex items-start gap-4 relative overflow-hidden group select-none cursor-pointer ${isActive ? 'bg-[#00d1ff]/10 border-[#00d1ff]/50 shadow-[0_0_15px_rgba(0,209,255,0.08)]' : 'bg-[#12141c]/90 border-[#2d3139]/70 hover:border-[#8e9299]/50'}`}
                            >
                              <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${isActive ? 'bg-[#00d1ff] text-black' : 'bg-white/5 text-[#8e9299] group-hover:text-white group-hover:bg-[#f3f4f6]/10'} transition-all`}>
                                <IconComp size={14} />
                              </div>

                              <div className="space-y-0.5 text-left">
                                <h5 className={`text-xs font-bold leading-tight ${isActive ? 'text-[#00d1ff]' : 'text-white'}`}>
                                  {item.label}
                                </h5>
                                <p className="text-[10px] text-[#8e9299] leading-snug font-medium">
                                  {item.desc}
                                </p>
                              </div>

                              {isActive && (
                                <div className="absolute right-3 top-3 w-1.5 h-1.5 rounded-full bg-[#00d1ff] shadow-[0_0_8px_#00d1ff]" />
                              )}
                            </button>
                          );
                        })}
                ...
                {CATEGORIES_MAP.every(category => 
                  ...
                  <div className="text-center py-12 space-y-2">
                    <div className="text-slate-500 font-mono text-xs">NO PIPELINES MATCHING YOUR SEARCH</div>
                    <button 
                      onClick={() => setSearchText('')}
                      className="px-4 py-1.5 bg-white/5 border border-[#2d3139] rounded-lg text-[10px] text-white hover:bg-[#ffffff]/10 font-bold"
                    >
                      RESET SEARCH FILTER
                    </button>
                  </div>
